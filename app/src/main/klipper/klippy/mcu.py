...
class MCURestartHelper:
    def __init__(self, config, conn_helper):
        ...
        # Beam changed: always *consume* 'restart_method' from the config, even on
        # the virtual-serial path (baud == 0). Upstream only reads it when baud is
        # truthy, but Beam forces the MCU onto a pipe with baud 0, so the option
        # would go unread and klippy's check_unused_options() would abort startup
        # with "Option 'restart_method' is not valid in section 'mcu'" for any
        # otherwise-valid printer.cfg that sets it. It is still only *applied* for
        # a real UART connection.
        cfg_restart_method = config.getchoice('restart_method',
