...
class MetadataStorage:
    ...
    async def _run_extract_metadata(self,
        ...
        eventloop = self.server.get_event_loop()
        # Beam/Android: Chaquopy provides no spawnable `sys.executable`, so the
        # upstream "run metadata.py as a subprocess" path always fails and every
        # uploaded gcode showed up with no thumbnail / object list / estimates.
        # Run the extractor in-process on a worker thread instead.
        from .metadata import extract as _extract_metadata
        try:
            metadata: Dict[str, Any] = await asyncio.wait_for(
                eventloop.run_in_thread(_extract_metadata, config), timeout
            )
        except asyncio.TimeoutError:
            raise self.server.error("Extract Metadata timed out")
        if not metadata:
